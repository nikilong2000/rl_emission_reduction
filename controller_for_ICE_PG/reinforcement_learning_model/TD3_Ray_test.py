...
class RolloutWorker:
    ...
    def collect_windows(self):
        ...
        while True:
            ...
            if self.env.step_count < 5:  # Print the first 5 steps of the episode
                print(f"\n[Worker {self.worker_id} | Step {self.env.step_count}]")
                # Round physical actions so they are readable
                action_phys_rounded = np.round(action_phys, 2)
                print(f"  Physical Action (to env.step): {action_phys_rounded}")
                # Round observation so it is readable
                next_obs_rounded = np.round(next_obs, 2)
                print(f"  Next Obs (from env.step):     {next_obs_rounded}")
                print(f"  Reward (from env.step):       {reward:.4f}")
        ...
        # --- Logic for slicing the episode into windows ---
        windows = []
...
class TD3:
    ...
    def __init__(
        ...
        # --- LEARNER Optimizers ---
        self.optim_actor = optim.Adam(self.Actor.parameters(), lr=0.0003)
        self.optim_critic1 = optim.Adam(self.Critic1.parameters(), lr=0.0003)
        self.optim_critic2 = optim.Adam(self.Critic2.parameters(), lr=0.0003)
    ...
    def learn(
        ...
        print(
            f"🔥 Iniciando fase de calentamiento del buffer. Objetivo: {learning_starts} ventanas."
        )
        ...
        if self.reuse_warmup_buffer:
            ...
            if self._disk_window_count >= learning_starts:
                ...
                print(
                    f"✅ Warm-up cargado desde disco: {loaded}/{learning_starts} ventanas."
                )
        ...
        while timesteps_collected < total_timesteps:
            ...
            ):  # Evaluar cada 500 actualizaciones del crítico
    ...
    def compute_critic_loss(self, batch):
        ...
        if self.total_iterations == 0:
            ...
            for i in range(
                state_b.shape[2]
            ):  # Itera sobre la última dimensión (obs_dim=5)
                feat = state_b[..., i]
                name = state_names[i] if i < len(state_names) else f"Estado Idx {i}"
                print(
                    f"    {name}: min={feat.min():.2f}, max={feat.max():.2f}, mean={feat.mean():.2f}"
                )
            ...
            for i in range(
                action_b.shape[2]
            ):  # Itera sobre la última dimensión (act_dim=3)
                feat = action_b[..., i]
            ...
            print(
                f"    Rewards: min={reward_b.min():.2f}, max={reward_b.max():.2f}, mean={reward_b.mean():.2f}\n"
            )
        ...
        with torch.no_grad():
            ...
            if (
                self.total_iterations % 256 == 0
            ):  # Imprimir cada 256 iteraciones (1 ráfaga)
                print(f"\n--- DEBUG (Iter {self.total_iterations}): Target Critic ---")
                print("  Target Actions (pre-ruido):")
                for i in range(
                    actions_target_unroll.shape[-1]
                ):  # Itera sobre la última dim (act_dim)
                    feat = actions_target_unroll[..., i]
                    print(
                        f"    Acción {i}: min={feat.min():.2f}, max={feat.max():.2f}, mean={feat.mean():.2f}"
                    )
        ...
        if self.total_iterations % 256 == 0:
            print(
                f"  Critic Loss 1: {loss1.item():.4f} | Critic Loss 2: {loss2.item():.4f}"
            )
        # --- FIN DEBUG ---
        ...
        return (
            loss1,
            loss2,
        )  # <--- ASEGÚRATE DE QUE ESTA LÍNEA EXISTE Y ESTÁ INDENTADA AL NIVEL DE LA FUNCIÓN
    ...
    def compute_actor_loss(self, batch):  # CORRECTE
        ...
        # --- DEBUG PRINT: SATURACIÓN DEL ACTOR PRINCIPAL (POR CARACTERÍSTICA) ---
        if self.total_iterations % 256 == 0:
            print(f"--- DEBUG (Iter {self.total_iterations}): Main Actor ---")
            print("  Main Actor Actions (pre-eval):")
            for i in range(
                actions_pred_unroll.shape[-1]
            ):  # Itera sobre la última dim (act_dim)
    ...
    def _init_or_check_buffer_meta(self):
        ...
        if not os.path.exists(self._buffer_meta_path):
            ...
            try:
                ...
                if not ok:
                    print(
                        "[WARN] El buffer en disco fue creado con otra configuración "
