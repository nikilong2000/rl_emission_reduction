...
os.environ["ORT_LOGGING_LEVEL"] = "4"  # Only critical errors

# Import NEW CODE classes
...
def setup_transition_function_model(ruta_ICE, ruta_PG, SOC_ini=0.7):
    """
    Configures the transition environment by loading the ONNX models and their scalers.
    """
    print(f"⚙️ Configuring ONNX environment...")
    print(f"   📍 ICE: {ruta_ICE}")
    print(f"   📍 PG:  {ruta_PG}")

    # --- Load ICE Model and Scalers ---
    # Assuming the model file is named 'ICE_onnx.onnx' based on your example
...
class transition_function_model:
    def __init__(
        ...
        # Save initial physical values to reset later
        ...
        # Then reshape to (1, 1, features) to fit the model.
    ...
    def reset_models(self):
        """
        Resets the LSTM internal states and auxiliary variables.
        """
        # 1. Reset internal states of the LSTM_onnx class
