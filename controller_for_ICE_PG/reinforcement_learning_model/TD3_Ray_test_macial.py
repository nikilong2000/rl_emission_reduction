...
from environment_macial import Environment
...
"""
COSES A REVISAR:
- ENTRADA MODEL SIGUI VALORS REALS
"""
...
class RolloutWorker:
    ...
    """
    def __init__(self, model_paths, worker_id, scaler_params, act_dim, length_problem, vel_target, B, U):
        
        ice_folder, pg_folder = model_paths
        
        print(f"Worker {worker_id}: Cargando modelos ONNX...")
        
        # Aquí es donde se crea la sesión de ONNX segura para este proceso
        self.f_transicio = setup_transition_function_model(ice_folder, pg_folder)
        
        
        self.vel_target=vel_target
        self._silence_tf_in_worker() #Correcte
        self.worker_id = worker_id #Correcte
        
        # Parámetros para trocear secuencias 
        self.B, self.U = B, U #B=calentamiento, U=entrenamiento Correcte
        self.S = B + U #S = ventana total Correcte
        
        # CAMBIO: n_stable = S + 50 para garantizar al menos 1 ventana por episodio
        # Esto evita que episodios cortos no generen ventanas de entrenamiento
        n_stable_dynamic = self.S + 50
        self.env = Environment(self.f_transicio, length_problem=length_problem, n_stable=n_stable_dynamic)
        
        # CAMBIO: Sigma inicial MUY alto para facilitar exploración
        self.exploration_noise = NormalActionNoise(mean=np.zeros(act_dim), sigma=0.3)
        self.vel_target = vel_target
        self.act_dim = act_dim  # Guardar para set_exploration_sigma
    ...
    def set_exploration_sigma(self, sigma):
        """Actualiza el sigma del ruido de exploración."""
        self.exploration_noise = NormalActionNoise(mean=np.zeros(self.act_dim), sigma=sigma)
        print(f"[Worker {self.worker_id}] Sigma de exploración actualizado a {sigma}")
    ...
    def descale_action(self, action_scaled):
        ...
        # action_01 = (action_scaled_np + 1.0) / 2.0
        # Aplicar la fórmula inversa del MinMaxScaler
    ...
    def _silence_tf_in_worker(self):
        ...
        Este método tiene dos objetivos principales:
            1. Silenciar los logs de TensorFlow: Evita que TensorFlow imprima mensajes informativos (INFO) o advertencias (WARNING) en la consola.
            2. Deshabilitar el uso de la GPU: Fuerza a TensorFlow a usar únicamente la CPU en este proceso.
        """
    ...
    def collect_windows(self):
        ...
        max_vel_reached = 0.0  # Tracking de velocidad máxima
        ...
        while True:
            ...
            # Actualizar velocidad máxima alcanzada
            vel_actual = next_obs[1]
            max_vel_reached = max(max_vel_reached, vel_actual)
                        
            # -----------------------------------------------------------
            # MONITOR SIMPLE Y CLARO (Solo Worker 0 + Aleatorio)
            # -----------------------------------------------------------
            # Imprime si es el Worker 0 Y un 1% aleatorio (aumentado para diagnóstico)
            if self.worker_id == 0 and (random.random() < 0.0025):  # Reducido a 0.1%
                
                # Extraemos los valores clave para no ver una lista de números sin sentido
                vel_actual = next_obs[1]
                vel_target = next_obs[0]
                
                # Acciones físicas: [Fuel, Brake, ICE_SP]
                gas   = action_phys[0]
                freno = action_phys[1]
                rpm   = action_phys[2]

                # Un print simple en una línea: [Paso] Velocidad | Acciones | Recompensa
                print(f"[Step {self.env.step_count:04d}] "
                      f"Vel: {vel_actual:5.1f}/{vel_target:.0f} km/h | "
                      f"Gas: {gas:5.1f}  Freno: {freno:5.1f}  RPM: {rpm:4.0f} | "
                      f"R: {reward:+.3f}")
            # -----------------------------------------------------------
        ...
        # Devolver tupla: (ventanas, ratio de velocidad máxima vs target)
        max_vel_ratio = max_vel_reached / self.vel_target
        return (windows, max_vel_ratio)


class TD3:
    """The TD3 Agent (Learner).
    
            1. Recolecta de datos: en buffer (s, a, r, s', d)
            2. Actualizar Critic: Los critics intentan predecir el Q-Value, que es la recompensa de ahora r más todas las recompensas futuras descontadas.
            3. Actualizar Actor: Mira un (s, a, r, s', d) del buffe. Predice que accion elegiria el, dado el estado, añadiendo ruido, El Critic evalúa esa propuesta. El Actor se actualiza para maximizar ese Valor.
            
                
    """

    def __init__(self, model_paths, version="test", act_dim=3, obs_dim=6, length_problem=1200, replay_size=1000000, batch_size=256, gamma=0.99, tau=0.001, policy_noise=0.3, noise_clip=0.5, policy_delay=2, early_stop=50, numeric_eval=False, scaler_params=None, vel_target=70, num_workers=2, U=300, B=150, reuse_warmup_buffer = False, buffer_dir= "buffer"):
        ...
        policy_noise : float, optional
            Standard deviation of the Gaussian noise added to the target actor's action for smoothing, defaults to 0.3.
        ...
        num_workers : int, optional
            Number of parallel 'worker' processes for experience collection, defaults to 0.2
        U : int, optional
        ...
        print("Configurando el correcto...")

        # --- Parámetros de la clase (la mayoría sin cambios) ---
        self.f_transicio = t_function
        ...
        self.first_convergence_handled = False  # Flag para la lógica de buffer clearing
        ...
        self.optim_critic1 = optim.Adam(self.Critic1.parameters(), lr=0.0001)
        self.optim_critic2 = optim.Adam(self.Critic2.parameters(), lr=0.0001)
        ...
        # Flag para detectar primera convergencia y vaciar buffer antiguo
        self.first_convergence_handled = False
    ...
    def learn(self, total_timesteps, learning_starts=1000, train_freq=4, gradient_steps=4):
        ...
        while len(self.replay_buffer) < learning_starts:
            ...
            # CAMBIO: Desempaquetar tupla (windows, episode_converged)
            result = ray.get(ready_ref)
            result_windows, _ = result  # Ignoramos episode_converged en warm-up
...
#             print(f"\rIteraciones: {self.total_iterations} | Timesteps: ~{timesteps_collected}/{total_timesteps} | Buffer: {len(self.replay_buffer)} | Pasos para entrenar: {steps_since_last_train}/{train_freq}", end="")
            ...
            if len(self.replay_buffer) > self.N and steps_since_last_train >= train_freq:
                ...
                for _ in range(gradient_steps):
                    ...
                    if self.total_iterations % self.policy_delay == 0:
                        ...
                        actor_loss, actions_for_grad = self.compute_actor_loss(batch)
                        self.optim_actor.zero_grad(set_to_none=True)
                        actor_loss.backward()
                        
                        # --- DEBUG: GRADIENTES POR DIMENSIÓN DE ACCIÓN ---
                        if self.total_iterations % 256 == 0 and actions_for_grad.grad is not None:
                            grad = actions_for_grad.grad
                            print(f"\n  --- Gradientes por Acción (después de backward) ---")
                            for i, name in enumerate(["mf", "brk", "ice_sp"]):
                                g = grad[..., i]
                                print(f"    {name}: mean={g.mean().item():.6f}, std={g.std().item():.6f}, max={g.abs().max().item():.6f}")
                            print()
                        # --- FIN DEBUG ---
            ...
            if ready_tasks_refs:
                ...
                # CAMBIO: Desempaquetar tupla (windows, max_vel_ratio)
                result = ray.get(ready_ref)
                worker_windows, max_vel_ratio = result
                ...
                # CAMBIO: Vaciar 30% del buffer y reducir ruido cuando alcance 35% del target
                if max_vel_ratio >= 0.35 and not self.first_convergence_handled:
                    n_removed = self.replay_buffer.prune_oldest(0.3)
                    print(f"\n🎯 [35% TARGET ALCANZADO] Max vel ratio: {max_vel_ratio:.2f}")
                    print(f"   Buffer purgado: {n_removed} ventanas antiguas eliminadas.")
                    
                    # Reducir ruido de exploración en todos los workers
                    for w in self.workers:
                        ray.get(w.set_exploration_sigma.remote(0.3))
                    print("📉 Ruido de exploración reducido de 0.8 a 0.3")
                    
                    self.first_convergence_handled = True
    ...
    def compute_critic_loss(self, batch):
        ...
        # --- DEBUG PRINT: RANGO DE DATOS (POR CARACTERÍSTICA) ---
        if self.total_iterations == 0:
            print("\n--- DEBUG: Datos del Primer Batch ---")
            
            # Nombres de las 5 características del estado (ajústalos si es necesario)
            state_names = ["vel_target", "vel_actual", "mf", "brk", "ice_sp"]
            print("  --- Estados (físicos) ---")
            for i in range(state_b.shape[2]): # Itera sobre la última dimensión (obs_dim=5)
                feat = state_b[..., i]
                name = state_names[i] if i < len(state_names) else f"Estado Idx {i}"
                print(f"    {name}: min={feat.min():.2f}, max={feat.max():.2f}, mean={feat.mean():.2f}")

            # Nombres de las 3 características de la acción
            action_names = ["mf_scaled", "brk_scaled", "ice_sp_scaled"]
            print("  --- Acciones (escaladas) ---")
            for i in range(action_b.shape[2]): # Itera sobre la última dimensión (act_dim=3)
                feat = action_b[..., i]
                name = action_names[i] if i < len(action_names) else f"Acción Idx {i}"
                print(f"    {name}: min={feat.min():.2f}, max={feat.max():.2f}, mean={feat.mean():.2f}")
                
            print("  --- Recompensas (reales) ---")
            print(f"    Rewards: min={reward_b.min():.2f}, max={reward_b.max():.2f}, mean={reward_b.mean():.2f}\n")
        
        # --- MONITOREO PERIÓDICO DE RECOMPENSAS ---
        if self.total_iterations % 256 == 0:
            print(f"\n  [Batch Rewards] min={reward_b.min():.2f}, max={reward_b.max():.2f}, mean={reward_b.mean():.2f}")
        
        # --- 2. CÁLCULO DEL OBJETIVO (TARGET Q) ---
        # Todo este bloque calcula y_i_unroll (el objetivo "caliente")
        # Se hace CON gestión de memoria (TBPTT) y SIN gradientes.
        with torch.no_grad():
            # A. Target Burn-in: Calentar la memoria de las redes TARGET
            #    (Usamos next_states_seq, o S_t+1)
            next_states_burn_in = next_states_seq[:, :self.B, :]
            
            # 1. Calentar Actor Target (necesitamos sus acciones para calentar a los Críticos Target)
            actions_target_burn_in, h_actor_target = self.Actor_target(next_states_burn_in)
            
            # 2. Calentar Críticos Target (usando las acciones del Actor Target)
            _, h_critic1_target = self.Critic1_target(next_states_burn_in, actions_target_burn_in)
            _, h_critic2_target = self.Critic2_target(next_states_burn_in, actions_target_burn_in)

            # B. Target Unroll: Calcular el valor objetivo para la parte 'U'
            next_states_unroll = next_states_seq[:, self.B:, :]
            rewards_unroll = rewards_seq[:, self.B:, :]
            done_unroll = done_seq[:, self.B:, :]

            # 1. Obtener acciones del Actor Target, partiendo de su memoria caliente
            actions_target_unroll, _ = self.Actor_target(next_states_unroll, hidden_state=h_actor_target)

            # 2. Aplicar "Target Policy Smoothing" (ruido TD3)
            raw_noise = torch.normal(0.0, self.policy_noise, size=actions_target_unroll.shape, device=self.device)
            if self.total_iterations % 256 == 0: # Imprimir cada 256 iteraciones (1 ráfaga)
                print(f"\n--- DEBUG (Iter {self.total_iterations}): Target Critic ---")
                print("  Target Actions (pre-ruido):")
                for i in range(actions_target_unroll.shape[-1]): # Itera sobre la última dim (act_dim)
                    feat = actions_target_unroll[..., i]
                    print(f"    Acción {i}: min={feat.min():.2f}, max={feat.max():.2f}, mean={feat.mean():.2f}")

            # AÑADE ESTA LÍNEA:
            noise = raw_noise.clamp(-self.noise_clip, self.noise_clip)
            next_actions_unroll = torch.clamp(actions_target_unroll + noise, -1.0, 1.0)
            
            # --- DEBUG PRINT ADICIONAL (TU IDEA) ---
            if self.total_iterations % 256 == 0:
                print("  Target Actions (post-ruido y post-clip):")
                for i in range(next_actions_unroll.shape[-1]):
                    feat = next_actions_unroll[..., i]
                    print(f"    Acción {i}: min={feat.min():.2f}, max={feat.max():.2f}, mean={feat.mean():.2f}")
            # --- FIN DEBUG ---
            
            # 3. Obtener Q-Targets, partiendo de la memoria caliente de los Críticos Target
            q1_t_unroll, _ = self.Critic1_target(next_states_unroll, next_actions_unroll, hidden_state=h_critic1_target)
            q2_t_unroll, _ = self.Critic2_target(next_states_unroll, next_actions_unroll, hidden_state=h_critic2_target)

            # 4. Calcular el objetivo final (Ecuación de Bellman)
            q_target_unroll = torch.min(q1_t_unroll, q2_t_unroll)
            
            # El objetivo final 'y_i_unroll' ya está "caliente" y solo para la parte 'U'
            y_i_unroll = rewards_unroll + (1 - done_unroll) * self.gamma * q_target_unroll
        ...
        # --- AÑADE ESTAS DOS LÍNEAS DE VUELTA ---
        ...
        # --- DEBUG PRINT: PÉRDIDAS ---
        if self.total_iterations % 256 == 0:
            print(f"  Critic Loss 1: {loss1.item():.4f} | Critic Loss 2: {loss2.item():.4f}")
        # --- FIN DEBUG ---
        
        # --- DEBUG VITAL ---
        if self.total_iterations % 256 == 0:
             # Queremos ver la magnitud de Q. Si es < 1.0, el agente no aprenderá nada.
             print(f"  > Q-Value Mean: {q1_unroll.mean().item():.4f} | Max: {q1_unroll.max().item():.4f}")
             print(f"  > Critic Loss 1: {loss1.item():.6f}") # Usar 6 decimales para ver detalles
        
        return loss1, loss2
    


    def compute_actor_loss(self, batch): #CORRECTE
            """
            Calculation of the actor's loss with TBPTT correctly implemented.
            Includes Action Regularization to prevent saturation.
            """
            ...
            # --- DEBUG PRINT: SATURACIÓN DEL ACTOR PRINCIPAL ---
            if self.total_iterations % 256 == 0:
                print(f"--- DEBUG (Iter {self.total_iterations}): Main Actor ---")
                print("  Main Actor Actions (pre-eval):")
                for i in range(actions_pred_unroll.shape[-1]): # Itera sobre la última dim (act_dim)
                    feat = actions_pred_unroll[..., i]
                    print(f"    Acción {i}: min={feat.min():.2f}, max={feat.max():.2f}, mean={feat.mean():.2f}")
            # --- FIN DEBUG ---

            # 4. Evaluar estas acciones con el Crítico. 
            with torch.no_grad():
            ...
            # Ahora sí, evaluamos las acciones del actor en el unroll
            ...
            # A. Pérdida Original (Maximizar Q)
            # El Actor aprende a elegir acciones que maximicen Q
            q_loss = -q_values_unroll.mean()
            
            # B. Regularización L2 (para evitar saturación)
            # Penaliza acciones extremas pero permite alcanzar los límites
            action_reg = 0.1 * torch.mean(actions_pred_unroll ** 2)
            
            # Pérdida Total
            actor_loss = q_loss + action_reg

            # --- DEBUG PRINT: PÉRDIDA DEL ACTOR ---
            if self.total_iterations % 256 == 0:
                print(f"  Actor Loss (Q): {q_loss.item():.4f}")
                print(f"  Actor Loss (Reg): {action_reg.item():.4f}")
                print(f"  Actor Loss (Total): {actor_loss.item():.4f}")
                # Monitorear saturación
                action_std = actions_pred_unroll.std().item()
                print(f"  Action Std Dev: {action_std:.4f} (0 = saturado, ~0.5 = bien)")
            # --- FIN DEBUG ---

            # --- DEBUG: GRADIENTES POR DIMENSIÓN DE ACCIÓN ---
            if self.total_iterations % 256 == 0:
                # Calcular gradientes manualmente para cada dimensión
                # Esto nos dice si el gradiente fluye hacia cada acción
                actions_pred_unroll.retain_grad()  # Permitir ver el gradiente
                
            for p in self.Critic1.parameters(): p.requires_grad_(True)

            return actor_loss, actions_pred_unroll  # Retornar acciones para analizar gradientes
...
#             n=1
            ...
            # 'returns' ahora almacenará las RECOMPENSAS MEDIAS de cada episodio
            returns = [] 
...
#                     print(f"reward {reward}, obs {obs}")
                ...
                # Evitar división por cero si el episodio terminase en 0 pasos (improbable)
                if episode_steps > 0:
