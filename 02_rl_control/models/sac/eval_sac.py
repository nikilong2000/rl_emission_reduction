...
#   python models/eval.py path/to/model.zip --algorithm sac
import os
import sys
import argparse
import datetime
...
def evaluate_model(
    ...
    plot_state_visitation_1d([eval_results], ["SAC"], log_dir)
    plot_state_visitation_2d(eval_results, log_dir)
    plot_action_distribution([eval_results], ["SAC"], log_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "model_path", type=str, help="Path to the trained SAC model (.zip)"
    )
    parser.add_argument(
        "--use_thermal",
        action="store_true",
