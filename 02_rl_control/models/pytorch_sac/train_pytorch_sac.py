...
try:
    ...
    from . import config
    from .eval_pytorch_sac import evaluate_model
...
def main(args):
    ...
    resolved_agent_device = resolve_torch_device(args.agent_device)

    base_log_dir = os.path.join(
        os.path.dirname(os.path.dirname(current_dir)), "logs", "pytorch_sac"
    )
    ...
    print(f"Torch device resolved to: {resolved_agent_device}")
    print(f"TorchRL modules available: {torchrl_available()}")
    ...
    train_config = {
        "algorithm": "PyTorch_TorchRL_Recurrent_SAC",
        ...
        "agent_device": resolved_agent_device,
    ...
    agent = TorchRLRecurrentSACAgent(
        obs_dim=obs_dim,
    ...
    for step in range(1, config.TOTAL_TIMESTEPS + 1):
        ...
        if step % config.CHECKPOINT_FREQ == 0:
            checkpoint_prefix = "pytorch_sac_emission_model"
    ...
    final_model_base = os.path.join(log_dir, "pytorch_sac_emission_final")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train or continue training a PyTorch TorchRL recurrent SAC model."
    )
    parser.add_argument(
        "--continue_from",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(e.g. 'cpu', 'cuda', 'auto'). "
            "Use 'cuda' to maximize GPU utilization when available."
        ),
    )
    parser.add_argument(
        "--use_thermal",
