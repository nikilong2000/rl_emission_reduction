...
def _density_contour(ax, xpts, ypts, extent, color, ls, lw=1.7, halo="black"):
    ...
    cs = ax.contour(
        xc, yc, h, levels=levels, colors=[color], linewidths=lw, linestyles=ls, zorder=5
    )
...
def plot_overlay(
    ...
    for ax, algo in zip(axes, ALGOS):
        ...
        if not m1.empty:
            _density_contour(
                ax,
                m1["ice_speed_rpm"].to_numpy(),
                m1["ice_torque"].to_numpy(),
                extent,
                "white",
                ":",
                1.7,
            ...
            ax.scatter(
                [m1["ice_speed_rpm"].median()],
                [m1["ice_torque"].median()],
                s=230,
                marker="X",
                color="white",
                edgecolors="black",
        ...
        if not m2.empty:
            _density_contour(
                ax,
                m2["ice_speed_rpm"].to_numpy(),
                m2["ice_torque"].to_numpy(),
                extent,
                color,
                "-",
                2.1,
            ...
            ax.scatter(
                [m2["ice_speed_rpm"].median()],
                [m2["ice_torque"].median()],
                s=240,
                marker="X",
                color=color,
                edgecolors="white",
        ...
        if not m2.empty:
            txt = (
                f"P2 median: {m2['ice_speed_rpm'].median():.0f} rpm, "
...
def plot_single(
    emap, quantity, label, cmap, out_path, logs, sub, phase_label, mark_extreme="max"
):
    ...
    for ax, algo in zip(axes, ALGOS):
        ...
        if not m.empty:
            _density_contour(
                ax,
                m["ice_speed_rpm"].to_numpy(),
                m["ice_torque"].to_numpy(),
                extent,
                color,
                "-",
                2.1,
            ...
            txt = (
                f"median: {m['ice_speed_rpm'].median():.0f} rpm, "
    ...
    handles = [
        Line2D(
            ...
            markeredgecolor="white",
            markersize=13,
            label=f"{phase_label} median op-point",
        ),
        Line2D(
            [0],
            [0],
...
def main():
    ...
    plot_overlay(
        ...
        "viridis_r",
        os.path.join(args.out_dir, "engine_map_bsfc_overlay.png"),
        args.phase1_logs,
        args.phase2_logs,
        args.phase1_subpath,
        args.phase2_subpath,
    ...
    plot_overlay(
        emap,
        "mnox",
        "NOx mass flow mNOx (g/h)",
        "inferno",
        os.path.join(args.out_dir, "engine_map_nox_overlay.png"),
        args.phase1_logs,
        args.phase2_logs,
        args.phase1_subpath,
        args.phase2_subpath,
