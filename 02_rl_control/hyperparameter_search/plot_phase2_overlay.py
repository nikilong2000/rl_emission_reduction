    ...
    for algo, style in ALGO_STYLE.items():
        ...
        if not s1.empty:
            ax.scatter(
                ...
                alpha=0.7,
                zorder=2,
                label=f"{style['label']} P1" if algo == "ppo" else None,
            )
        if not s2.empty:
            ax.scatter(
                s2["rmse_speed_kmph"],
...
def _density_contour(ax, xpts, ypts, extent, color, alpha=0.6):
    ...
    ax.contour(X, Y, Z, levels=levels, colors=[color],
               linewidths=[0.5, 0.9, 1.3, 1.6], alpha=alpha)


def plot_engine_map_overlay(
    phase1_logs: str, phase2_logs: str, bsfc: pd.DataFrame, out_path: str,
    p1_subpath: str, p2_subpath: str, algos: list[str],
):
    tri = mtri.Triangulation(bsfc["rpm"], bsfc["torque"])
    extent = [bsfc.rpm.min(), bsfc.rpm.max(), bsfc.torque.min(), bsfc.torque.max()]

    fig, axes = plt.subplots(1, len(algos), figsize=(5.2 * len(algos), 5.5), sharey=True)
    if len(algos) == 1:
        axes = [axes]
    ...
    for ax, algo in zip(axes, algos):
        c = ax.tricontourf(tri, bsfc["bsfc"], levels=levels, cmap="viridis_r",
                           alpha=0.85)
        ax.tricontour(tri, bsfc["bsfc"], levels=8, colors="k",
                      linewidths=0.4, alpha=0.4)
        ax.scatter([sweet["rpm"]], [sweet["torque"]], marker="*", s=160,
                   c="white", edgecolors="black", lw=0.6, zorder=6)
    ...
    fig.suptitle("Operating-point density on EA189 BSFC map — Phase-1 vs Phase-2",
                 fontsize=12)
...
def plot_tracking_overlay(
    phase1_logs: str, phase2_logs: str, out_path: str,
    p1_subpath: str, p2_subpath: str, algos: list[str],
):
    from glob import glob

    fig, axes = plt.subplots(len(algos), 1, figsize=(12, 3.4 * len(algos)),
                             sharex=True)
    if len(algos) == 1:
        axes = [axes]
    dt = 0.5
    ...
    for ax, algo in zip(axes, algos):
        ...
        if s1 is not None:
            ...
            ax.plot(t, err1.mean(axis=0), color="0.45", lw=1.2,
                    label=f"P1 mean (n={A1.shape[0]})")
            ax.fill_between(t, np.maximum(err1.mean(0) - err1.std(0), 0),
                            err1.mean(0) + err1.std(0),
                            color="0.7", alpha=0.4, label="P1 ±1σ")
        if s2 is not None:
            A2, T2 = s2
            err2 = np.abs(A2 - T2)
            t = np.arange(err2.shape[1]) * dt
            color = ALGO_STYLE[algo]["color"]
            ax.plot(t, err2.mean(axis=0), color=color, lw=1.4,
                    label=f"P2 mean (n={A2.shape[0]})")
            ax.fill_between(t, np.maximum(err2.mean(0) - err2.std(0), 0),
                            err2.mean(0) + err2.std(0),
                            color=color, alpha=0.25, label="P2 ±1σ")

        ax.set_ylabel("|Speed err| (km/h)")
        ax.set_title(f"{ALGO_STYLE[algo]['label']} — Phase-1 vs Phase-2 speed-tracking error")
...
def main():
    ...
    plot_pareto_overlay(df_p1, df_p2,
                        os.path.join(args.out_dir, "pareto_overlay.png"))

    bsfc = load_bsfc(args.engine_map)
    plot_engine_map_overlay(
        args.phase1_logs, args.phase2_logs, bsfc,
        os.path.join(args.out_dir, "engine_map_overlay.png"),
        args.phase1_subpath, args.phase2_subpath, args.algos,
    )
