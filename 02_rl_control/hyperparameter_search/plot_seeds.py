"""
Aggregate results from 10-seed validation runs and produce publication plots.
Outputs three figures:
  1. training_progress.png   —  mean reward (rolling 100 ep) ± std
  2. eval_rmse_tracking.png  —  speed tracking during eval episode ± std
  3. final_metrics_bar.png   —  bar chart of final metrics ± std
Usage:
...
# ---------------------------------------------------------------------------
# Plot 1 — Training progress (mean reward, rolling 100 episodes)
# ---------------------------------------------------------------------------
def plot_training_progress(seed_dirs, output_dir, algo_label="RL"):
    """Load monitor.csv from each seed, compute rolling-mean reward, plot mean±std."""
    all_curves = []
    for sd in seed_dirs:
        try:
            df = load_results(sd)
        ...
        rolling = df["r"].rolling(window=100, min_periods=1).mean().values
        all_curves.append(rolling)
    if not all_curves:
        print("No training data found — skipping training_progress plot.")
        return
    # Truncate to shortest curve
    ...
    ax.plot(episodes, mean, color=MEAN_COLOR, linewidth=1.2, label="Mean")
    ax.fill_between(episodes, mean - std, mean + std,
                    color=STD_COLOR, alpha=0.5, label="± 1 Std")
    _apply_style(ax, f"{algo_label} Training Progress (10 Seeds)",
                 "Episode (across all envs)", "Episodic Reward (rolling 100)")
...
def plot_eval_rmse_tracking(seed_dirs, output_dir, algo_label="RL"):
    ...
    # Target should be identical across seeds (deterministic eval schedule)
    ...
    ax.fill_between(time_s, actual_mean - actual_std, actual_mean + actual_std,
                    color=STD_COLOR, alpha=0.5, label="± 1 Std")
    _apply_style(ax, f"{algo_label} Evaluation — Speed Tracking (10 Seeds)",
                 "", "Speed (km/h)")
...
def plot_final_metrics_bar(seed_dirs, output_dir, algo_label="RL"):
    """Read evaluation_metrics.json from each seed. Bar chart with error bars."""
    records = []
    for sd in seed_dirs:
    ...
    for idx, (key, label, color) in enumerate(METRIC_KEYS):
        ...
        ax.bar(0, mean_val, width=0.5, color=color, alpha=0.85,
               yerr=std_val, capsize=8, ecolor="black", error_kw={"linewidth": 1.3})
        ax.set_xticks([])
        ...
        ax.text(0, mean_val + std_val * 1.15, f"{mean_val:.3f}±{std_val:.3f}",
                ha="center", va="bottom", fontsize=9, fontweight="bold")
    # Hide unused subplots
    for idx in range(n_metrics, len(axes)):
        axes[idx].set_visible(False)
    fig.suptitle(f"{algo_label} Final Evaluation Metrics (10 Seeds)",
...
def main(args):
    ...
    # Discover seed directories (seed_0, seed_1, ..., seed_9)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Aggregate 10-seed results into publication plots."
    )
    parser.add_argument(
        "--results_dir",
        type=str,
