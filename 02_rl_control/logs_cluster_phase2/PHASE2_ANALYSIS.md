...
1. *Can fine-tuning the phase-1 policy with an emission/SOC-augmented reward reduce NOx and bound SOC drift without sacrificing speed-tracking?*
2. *Which algorithm transfers best to the multi-objective regime, and what are the failure modes?*
...
> **WLTC caveat carries over.** The "phase low/medium/high/extra_high" labels in `calculate_emissions_per_km` are durational sub-windows over the staircase, not the regulatory WLTC speed schedule. *Indicative*, not regulatory.
...
A non-trivial finding for the thesis: **the recovered zombie trials do not change Optuna's selected best.** Best-trial parameters were already chosen before the zombies were recovered (the local journal already contained the eventual winners — trials 4 for SAC and TD3, trial 1 for PPO). The recovery served to fully populate the 20-trial budget rather than to find a better optimum.
...
### 5.5 Operating-point shift (BSFC engine map)

[analysis_plots/engine_map_overlay.png](analysis_plots/engine_map_overlay.png) shows per-algo operating-point density on the EA189 BSFC engine map. Comparing phase-1 (gray contours) to phase-2 (algo-colored contours):

* **PPO** shifts from a tight ~4 000 rpm / low-torque cruise to a broader distribution centred at ~3 000 rpm / mid-torque, much closer to the BSFC sweet spot (~2 500 rpm / 150 Nm).
* **SAC** shows the largest qualitative shift — phase-1 occupancy at ~3 100 rpm / 0 Nm migrates to **the sweet spot itself**, explaining the −93 % NOx and the low fuel.
* **TD3** retains a bimodal RPM distribution but the high-RPM mode (~3 700 rpm) becomes smaller relative to the low-RPM mode (~2 200 rpm). This is the structural reason TD3 still emits twice the NOx of PPO/SAC even after fine-tuning.

The phase-2 standalone version of this plot is at [analysis_plots/engine_map_occupancy.png](analysis_plots/engine_map_occupancy.png).

### 5.6 Pareto picture

[analysis_plots/pareto_overlay.png](analysis_plots/pareto_overlay.png) overlays the 30 phase-1 seeds (open gray markers) and 30 phase-2 seeds (filled, algo-coloured). Three observations:

1. **The entire phase-2 cloud sits strictly below the phase-1 Pareto front in the NOx dimension** for SAC and TD3, and tightly cluster on PPO's already-low NOx region. The phase-2 Pareto front (dashed black) is dominated by SAC seeds (lowest NOx) and PPO seeds (lowest RMSE).
2. **No phase-2 seed has higher NOx than the phase-1 cleanest seed.** The fine-tune is monotonic in NOx improvement.
3. **The TD3 seed 3 outlier appears at RMSE = 35.6 km/h** as a single far-right marker — visually obvious as the only departure from the phase-2 cluster.

The phase-2 standalone version is at [analysis_plots/pareto_front.png](analysis_plots/pareto_front.png).
...
In phase 1, PPO was the *least* reliable algorithm (σ_RMSE = 0.98 km/h, one outlier at 6.31). In phase 2, PPO is the *most* reliable (σ_RMSE = 0.07 km/h, zero outliers). Two complementary explanations:
...
SAC's combination of (a) off-policy replay carrying a *re-weighted* reward through the existing buffer, (b) the auto-tuned entropy temperature keeping exploration alive during fine-tune, and (c) the three-layer net learned in phase 1 having more capacity to fit a multi-objective surface — together give SAC the largest absolute and relative NOx reduction (4.51 g, −93 % vs phase-1's 48.94 g). The single 2.06 g seed-7 result is best-in-class.
...
* The critic must re-estimate Q under the new reward. Twin-critic minimum-target dampens the re-estimation but does not solve the *direction* of the gradient.
...
1. **WLTC caveat carries over from phase 1.** The "phase" labels are durational windows over the staircase, not the regulatory WLTC speed schedule. Any compliance claim should specify *"on the staircase cycle"*, not *"WLTC"*.
...
5. **`W_SOC_SQUARED = 0` and `W_FUEL = 0` were locked.** Other reward forms (linear ΔSOC, fuel-mg, BSFC-aware) are not explored. The current "best" is conditioned on this 2-D weight space.
...
* **Aftertreatment-aware env (phase 2.5).** Drivetrain_Plus LSTM logging is plumbed but `USE_ONNX = True` skips it. Enabling it for evaluation provides EM1/EM2 IST signals to argue about *actuator-level* emission shaping, not only setpoint-level.
...
* Plot scripts:
  ...
  * Overlay phase-1 vs phase-2: [hyperparameter_search/plot_phase2_overlay.py](../hyperparameter_search/plot_phase2_overlay.py).
...
The full result set required to reproduce this analysis is the set of `evaluation_metrics.json`, `evaluation_data.csv` and `train_config.json` files under [logs/<algo>/phase2_seeds/seed_<n>/](logs/) plus the three `best_params_phase2.json` and `all_trials.csv` files at [logs/<algo>/phase2_optuna/](logs/).
