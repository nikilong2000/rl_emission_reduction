...
\begin{itemize}
  \item \textbf{Velocity.} All algorithms achieve a mean cumulative reward within 5\% of the theoretical ceiling, confirming that the speed-tracking sub-task is solvable by all three approaches.
  \item \textbf{Nitrogen Oxides.} The mean \ac{NOx} emission per evaluation cycle (without an \ac{NOx} penalty in the reward) is between 48 and 95\,g, corresponding to roughly $1{,}200$--$2{,}300$\,mg/km when normalised by the cycle distance. This is 15--30$\times$ above the Euro~6 limit of $80$\,mg/km of \ac{NOx} and constitutes the upper bound that Phase~2 must reduce. The Euro~6 comparison is indicative rather than regulatory because the evaluation cycle is a step-wise staircase rather than a certification \ac{WLTC} run.
...
\ac{PPO} achieved its strongest training run of the hyperparameter optimisation at a cumulative reward of 3512.95 with the hyperparameter configuration listed in \autoref{tab:phase1_best_hyperparameters} in Appendix~\ref{app:phase1_best_sets}. The corresponding Optuna trial history is provided in \autoref{fig:phase1_ppo_trials}. Out of 70 sampled Optuna trials, only 17 ran to completion while the remaining 53 were terminated early by the median pruner. Optuna eventually concentrated the search on a narrow learning-rate band of approximately $10^{-5}$\,--\,$3\times 10^{-5}$, and the best-performing configuration sits at $1.28 \times 10^{-5}$. Median pruning was therefore essential in allocating computing resources to promising trajectories rather than running hopeless trials to completion. Most non-pruned runs converged near the theoretical reward maximum of $3{,}600$, with the steepest learning progress between roughly $500{,}000$ and $1{,}500{,}000$ steps and a plateau reached around $3{,}000{,}000$ steps. The selected configuration combines this unusually small learning rate with $n_{\text{epochs}} = 11$ over $2{,}048$ environment steps per update and a $[256, 256]$ \ac{MLP}, producing slow but stable policy updates.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{imgs/plots/phase1/ppo/eval_rmse_tracking.png}
...
\paragraph{Seed runs.}
The 10 seed runs for this hyperparameter configuration are shown in \autoref{fig:phase1_ppo_seeds}. All 10 seeds converge to cumulative training rewards between $3{,}351$ and $3{,}438$. Some seeds learn appreciably faster than others, which is solely due to the random initialisation since the hyperparameter configurations are identical across runs.

Overall, the high episodic rewards achieved during training across all 10 seeds yield the average speed-tracking trajectory shown in \autoref{fig:phase1_ppo_eval_rmse_tracking}. The mean \ac{RMSE} of $4.19 \pm 0.98$\,km/h and \ac{MAE} of $2.19 \pm 1.24$\,km/h confirm a strong ability to track the reference, although the per-seed dispersion is the largest of the three algorithms. The best individual seed reaches $3.16$\,km/h \ac{RMSE}, comparable to the SAC and TD3 best seeds, while a single outlier (seed 8) settles at $6.31$\,km/h \ac{RMSE} with $135$\,g of fuel and $110$\,g of \ac{NOx}.

The remaining aggregated evaluation metrics are reported in \autoref{tab:phase1_summary}. The mean cumulative fuel consumption is $79.6$\,g and the mean cumulative \ac{NOx} is $48.3$\,g over the 1{,}800\,s evaluation cycle, while the mean $\Delta\text{SOC} = +0.247$ reflects the strong tendency of PPO seeds to charge the battery up to the upper clip at $0.9999$. The implications of these values for Phase~2 are discussed in \autoref{subsec:results_phase1_summary}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{imgs/plots/phase1/ppo/seed_3_action_summary.png}
  \caption[Best PPO seed action summary in Phase~1]{Action summary of the best PPO seed in Phase~1.}
  \label{fig:phase1_ppo_action_summary}
\end{figure}

\paragraph{Action assessment for the best seed.}
The best PPO checkpoint is seed~3, selected by the lowest evaluation \ac{RMSE} among the PPO seeds. Its action summary in \autoref{fig:phase1_ppo_action_summary} shows a high-rpm strategy. The engine is almost exclusively on, while the \ac{ICE} speed is concentrated at $4{,}000$\,rpm basically across the full target-speed range. The \ac{EM2} torque remains close to zero with only mild positive torque in the $0$--$80$\,Nm range, the fuel command is narrowly concentrated around $17$--$23$\,mg, and the brake is essentially unused. This fixed operating point produces the cleanest best-seed policy of all three algorithms: $5.57$\,g cumulative \ac{NOx} and $74.46$\,g fuel at $3.16$\,km/h \ac{RMSE}. The result also shows that high engine speed alone is not the dominant \ac{NOx} driver; the integrated fuel seems more decisive.

\note{maybe add the correspodning evaluation plots; also do for TD3 and SAC}
...
\paragraph{Seed runs.}
The seed-wise learning curves (\autoref{fig:phase1_td3_seeds}) exhibit a fast initial climb to their plateau within the first $0.5$M steps. A single seed was not able to learn that fast initially and could only find a policy to cumulate the amount of reward at approximately $1{,}500{,}000$ steps. Different to \ac{PPO} however, is that the runs did not keep their stability once they reached the plateu; instead some of them seem to drift off to a worse policy and then recover again or end with a slightly lower episode reward.

TD3 produces the tightest \ac{RMSE} distribution of the three algorithms ($3.81 \pm 0.29$\,km/h, \autoref{fig:phase1_td3_eval_rmse_tracking}) but at the same time the widest energy-side distribution ($95.4 \pm 100.8$\,g \ac{NOx}, $122.6 \pm 56.9$\,g fuel). The mean $\Delta\text{SOC}$ of $+0.146$ also hides a bimodal underlying distribution: seeds $0$, $6$, and $8$ deplete the battery ($\Delta\text{SOC} = -0.12$, $-0.49$, $-0.03$ respectively), whereas the remaining seven seeds saturate it. Both groups can track the reference speed similarly well, which explains why the \ac{RMSE} distribution remains narrow while the energy- and emission-side distributions are much wider.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{imgs/plots/phase1/td3/seed_1_action_summary.png}
  \caption[Best TD3 seed action summary in Phase~1]{Action summary of the best TD3 seed in Phase~1.}
  \label{fig:phase1_td3_action_summary}
\end{figure}

\paragraph{Action assessment for the best seed.}
The best TD3 checkpoint is seed~1. Its action summary in \autoref{fig:phase1_td3_action_summary} shows a lower-rpm policy than \ac{PPO}, combined with a bimodal fuel distribution and highly volatile \ac{EM2} usage. The engine is again almost exclusively on, while the \ac{ICE} speed is concentrated mainly around $900$--$1{,}300$\,rpm and $2{,}000$--$2{,}500$\,rpm. The $4{,}000$\,rpm region is used only when the target speed in the evaluation cycle increases. During steady cruising periods, where the target speed remains constant, the \ac{EM2} torque is approximately $0$\,Nm. In the first staircase segment (up to step 600), it is exclusively negative at around $-165$\,Nm. In the fourth segment, at the highest target velocity of $140$\,km/h, the \ac{EM2} torque becomes highly volatile and alternates between approximately $+330$\,Nm and $-250$\,Nm every other time step (0.5\,s). Fuel injection is clearly bimodal, with one cluster around $10$--$20$\,mg and another around $50$--$70$\,mg. This high-fuel cluster gives TD3 the worst best-seed fuel and emission values: $163.94$\,g fuel and $73.42$\,g cumulative \ac{NOx} at $3.52$\,km/h \ac{RMSE}.
...
\paragraph{Seed runs.}
The seed-wise learning curves (\autoref{fig:phase1_sac_seeds}) reach $\approx 3{,}500$ within $0.3$M steps and remain at that level for the remainder of training, with two dips around $3.3$M steps. \hl{This is the most sample-efficient learning behaviour among the three algorithms, consistent with the high replay-update ratio.} The resulting evaluation \ac{RMSE} ($3.45 \pm 0.43$\,km/h) is both the lowest mean and the lowest per-seed variance of the three algorithms (see \autoref{tab:phase1_summary}), making SAC the most reliable speed tracker in Phase~1.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{imgs/plots/phase1/sac/seed_3_action_summary.png}
  \caption[Best SAC seed action summary in Phase~1]{Action summary of the best SAC seed in Phase~1.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{imgs/plots/phase1/training_progress_comparison.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{imgs/analysis_plots/parallel_coords.png}
...
% Phase~2 must reduce the uncontrolled Phase~1 side effects while maintaining comparable speed tracking. The concrete per-agent reference values to beat are therefore:

% \begin{itemize}
%   \item PPO should improve on $3.16$\,km/h \ac{RMSE}, $5.57$\,g \ac{NOx}, $74.46$\,g fuel and $\Delta\text{SOC}=+0.299$,
%   \item SAC should improve on $3.11$\,km/h \ac{RMSE}, $48.94$\,g \ac{NOx}, $109.31$\,g fuel and $\Delta\text{SOC}=+0.299$, and
%   \item TD3 should improve on $3.52$\,km/h \ac{RMSE}, $73.42$\,g \ac{NOx}, $163.94$\,g fuel and $\Delta\text{SOC}=+0.300$.
% \end{itemize}

% For \ac{SOC}, improvement means moving $\Delta\text{SOC}$ toward $0$ rather than further saturating or depleting the battery.
...
\paragraph{Weight selection by Optuna.} The two active Phase~2 weights are selected with the constrained Optuna objective defined in \autoref{eq:phase2_objective}, which minimises cumulative \ac{NOx} inside the admissible tracking and charge-drift region and penalises violations outside it. Each agent was tuned over 20 valid trials; the selected trials, scores and evaluation metrics are listed in \autoref{tab:phase2_best_trials}. The selected configuration was then re-trained over 10 seeds, and all reported statistics are the mean $\pm$ standard deviation over those seeds.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}llrrrrrr@{}}
    \hline
    \textbf{Algorithm} & \textbf{Trial} & \textbf{$\mathbf{W_{\text{emi}}}$} & \textbf{$\mathbf{W_{\text{soc}}}$} & \textbf{Score} & \textbf{\shortstack{\ac{RMSE} (km/h)}} & \textbf{\shortstack{\ac{NOx} (g)}} & \textbf{\shortstack{$\mathbf{\max|\Delta}$\ac{SOC}$|$}} \\
    \hline
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \hline
    Cumulative reward            & $3{,}519 \pm \phantom{0}41$ & $3{,}360 \pm 207$ & $3{,}461 \pm \phantom{0}51$ & $3{,}520 \pm \phantom{0}51$ \\
    \ac{RMSE} speed (km/h)       & $\mathbf{3.13 \pm 0.07}$ & $7.41 \pm 9.94$ & $3.96 \pm 0.47$ & $3.33 \pm 0.33$ \\
    \ac{MAE} speed (km/h)        & $1.55 \pm 0.05$ & --- & $1.91 \pm 0.45$ & $\mathbf{1.43 \pm 0.46}$ \\
    Cumulative \ac{NOx} (g)      & $4.94 \pm 0.72$ & $10.01 \pm 4.85$ & $8.65 \pm 4.43$ & $\mathbf{4.51 \pm 1.46}$ \\
    \ac{NOx} intensity (mg/km)   & $122$ & $250$ & $216$ & $\mathbf{111}$ \\
    Cumulative fuel (g)          & $\mathbf{52.8 \pm 1.3}$ & $79.8 \pm 31.4$ & $79.5 \pm 35.6$ & $64.0 \pm 20.2$ \\
    $|\Delta$\ac{SOC}$|$         & $0.029 \pm 0.004$ & $0.061 \pm 0.032$ & $0.072 \pm 0.034$ & $\mathbf{0.010 \pm 0.008}$ \\
...
\paragraph{Seed runs.}
PPO is the most reliable agent in Phase~2. After the initial reward shock, all 10 seeds recover to nearly identical learning curves (\autoref{fig:phase2_ppo_seeds}) and the evaluation \ac{RMSE} collapses to $3.13 \pm 0.07$\,km/h --- below the Phase~1 10-seed mean of $4.19$\,km/h, and with a per-seed standard deviation more than an order of magnitude tighter than the $0.98$\,km/h of Phase~1. The speed-tracking trajectory in \autoref{fig:phase2_ppo_eval_rmse_tracking} is essentially indistinguishable from the Phase~1 PPO baseline, while the evaluation metrics in \autoref{tab:phase2_summary} show the energy side transformed: mean cumulative \ac{NOx} falls to $4.94 \pm 0.72$\,g (from the Phase~1 10-seed mean of $48.3$\,g) and fuel to $52.8 \pm 1.3$\,g (from $79.6$\,g), and the charge drift tightens to $|\Delta\text{SOC}| = 0.029$ from the Phase~1 10-seed mean of $+0.247$. PPO therefore converts an already-clean but battery-saturating Phase~1 policy into a clean and charge-sustaining one at no cost to tracking accuracy.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{imgs/plots/phase2/ppo/eval_rmse_tracking.png}
...
\paragraph{Reward-weight search.}
TD3's study ran 20 valid trials (15 completed, 5 pruned) and selected the lowest \ac{SOC} weight of the three agents, $W_{\text{soc}^2} = 64.50$, paired with a high emission weight $W_{\text{emi}} = 1.010$. The comparably low \ac{SOC} weight leads to the largest residual drift of the three agents ($\max|\Delta\text{SOC}| = 0.086$, \autoref{tab:phase2_best_trials}).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{imgs/plots/phase2/training_progress_comparison.png}
...
Activating the emission and \ac{SOC} reward terms and fine-tuning from the Phase~1 checkpoints reduces cumulative \ac{NOx} and the \ac{SOC} drift by roughly an order of magnitude (when neglecting TD3). Because every Phase~2 seed departs from the single best Phase~1 checkpoint, the like-for-like baseline for the resulting 10-seed statistics is the Phase~1 10-seed mean (\autoref{tab:phase1_summary}): measured against it, mean speed tracking is not degraded but slightly improved for the two stable agents (PPO $4.19 \to 3.13$, SAC $3.45 \to 3.33$\,km/h), with the seed-to-seed \ac{RMSE} variance reduced. The per-agent change against these Phase~1 10-seed baselines is summarised in \autoref{tab:phase2_headline}.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \ac{NOx} (mg/km)     & $1{,}165 \to 122$ & $2{,}327 \to 216$ & $1{,}564 \to 111$ \\
    Fuel (g)             & $79.6 \to 52.8$ & $122.6 \to 79.5$ & $113.5 \to 64.0$ \\
    $|\Delta$\ac{SOC}$|$ & $0.247 \to \mathbf{0.029}$ & $0.146 \to 0.072$ & $0.191 \to \mathbf{0.010}$ \\
    \hline
  \end{tabular}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{imgs/analysis_plots/phase2/pareto_overlay_soc.png}
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.49\textwidth}
    ...
    \caption[NOx rate versus engine torque]{Pooled tail-pipe \ac{NOx} rate as a function of engine torque for both phases and engine-on steps, with the inter-quartile band. The monotonic trend explains why moving to lower engine load reduces \ac{NOx}.}
...
more tightly than the rule-based controller (PPO RMSE $0.81$ vs $2.29$\,km/h),
demonstrating that the learned policies transfer zero-shot to an unseen, realistic
cycle. However, they do so at higher tailpipe-NOx and fuel cost: the
...
The selected seeds reverse the unfavourable fleet-level reading. \textbf{SAC seed~7 dominates the rule-based controller on the two emission-relevant axes simultaneously}: it cuts tailpipe NOx by $\sim$45\% (80\,mg/km vs. 147\,mg/km, i.e.\ down to the Euro-6 limit) while tracking the target more accurately (RMSE $1.76$ vs.\ $2.29$\,km/h) and remaining charge-sustaining ($\Delta\text{SOC}=+0.010$). It achieves this by switching the engine off $57\%$ of the time --- far more than the rule-based $16\%$ --- at the price of $\sim$26\% more burned fuel. PPO seed~2, by contrast, attains the best speed tracking of all controllers ($0.94$\,km/h) but does not beat the rule-based
baseline on NOx and retains the always-on, high-fuel behaviour. SAC seed~7 is therefore the selected candidate for continued development.

\note{add action history for that sac seed}
...
\begin{figure}[htbp]
  ...
  \caption[WLTC speed-tracking comparison]{Speed tracking on the WLTC. RL mean\,$\pm$\,$1\sigma$ across ten seeds per algorithm vs. the rule-based controller. Both RL algorithms follow the target trace closely across the full cycle.}
  \label{fig:wltc-tracking}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{imgs/plots/wltc_comparison/pareto_wltc.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{imgs/plots/wltc_comparison/phase_nox_wltc.png}
