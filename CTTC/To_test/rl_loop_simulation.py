...
os.environ["CUDA_VISIBLE_DEVICES"] = "-1"  # Force CPU usage for TF (remove if GPU is desired and available)
...
# ======================================================================
# Define which type of model to use -- TF-2.18 or CTTC
model_family = "TF-2.18" if len(sys.argv) < 2 else sys.argv[1].upper()  # Options: "TF-2.18", "CTTC"
print(f"Using model family: {model_family}")
# ======================================================================

if model_family == "TF-2.18":
    sys.path.append(
        os.path.join(current_dir, "..", "..", "internal_lstm_models", "NN_Application")
    )

    # Import helper functions from Modular_NN
    from Modular_NN import load_network, set_states, load_scaler # type: ignore

    # Path configurations for TF-2.18 models
    models_folder = os.path.join(current_dir, "..", "..", "internal_lstm_models", "NN_Application", "Nets")

elif model_family == "CTTC":
    # Import helper functions from ONNX_Predict
    from ONNX_Predict.utilities import load_network, set_states

    # Path configurations for CTTC models
    models_folder = os.path.join(current_dir, "..", "Models")

else:
    raise ValueError(f"Invalid model family: {model_family}. Choose 'TF-2.18' or 'CTTC'.")
# ======================================================================

# Configuration
ICE_MODEL_DIR = os.path.join(models_folder, "ICE")
PG_MODEL_DIR = os.path.join(models_folder, "Drivetrain")

data_folder = os.path.join(current_dir, "..", "..", "internal_lstm_models", "Test_Cycles")
data_family = "" if len(sys.argv) < 3 else sys.argv[2].lower()  # Optional argument to specify data family if needed
data = f"WLTC_{data_family}.csv" if data_family else "WLTC.csv"  # Default to WLTC.csv if no family specified
INPUT_DATA_PATH = os.path.join(data_folder, data)

OUTPUT_DIR = os.path.join(current_dir, "Output", model_family)
...
def main():
    ...
    ice_init_vals = np.array([ICE_INITIAL_OUTPUTS], dtype=np.float32).reshape(1, len(ICE_INITIAL_OUTPUTS))
    ...
    # PG Initialization
    pg_init_vals = np.array([PG_INITIAL_OUTPUTS], dtype=np.float32).reshape(1, len(PG_INITIAL_OUTPUTS))
    ...
    for t in range(num_steps):
        ...
        ice_pred = ice_out_scaler.inverse_transform(ice_pred_scaled[0])
        ...
        pg_pred = pg_out_scaler.inverse_transform(pg_pred_scaled[0])
