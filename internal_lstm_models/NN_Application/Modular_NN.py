...
def predict_with_lstm_network(
    ...
    input_data_scaled = input_data_scaled.reshape(
        (1, input_data_scaled.shape[0], input_data_scaled.shape[1])
    )
    ...
    if aux_initial_values is not None and timestep_global == 0:
        aux_input_data = output_scaler.transform(aux_initial_values).reshape(
            (1, 1, len(aux_initial_values[0]))
        )
...
def load_inputs_outputs_from_csv(csv_path, config, delimiter=[";", ","]):
    ...
    if df is None:
        raise ValueError(
            f"CSV konnte mit keinem der Delimiter {delimiter} geladen werden."
        ) from last_exception
...
def run_chained_networks(config, initial_inputs, true_outputs):
    ...
    for t in range(num_time_steps):
        current_inputs = {
            key: value[t : t + 1] for key, value in initial_inputs.items()
        }
        ...
        for name, details in config.items():
            ...
            print(
                f"Timestep {t} | Network: {name} | CSV inputs: "
            ...
            for input_index, input_source in enumerate(details["inputs"]):
                if ":" in input_source:
                    network_name, output_name = [
                        x.strip() for x in input_source.split(":")
                    ]

                    if (
                        output_name in network_outputs[network_name]
                        and len(network_outputs[network_name][output_name]) > t
                    ):
                        # Output[t] ist schon berechnet → direkt verwenden
                        print(
                            f"[t={t}] Using current output from {network_name}:{output_name}"
                        )
                        inputs.append(
                            network_outputs[network_name][output_name][t : t + 1]
                        )
                    elif (
                        t == 0
                        and "manual_initialization" in details
                        and input_index in details["manual_initialization"]
                    ):
                        # Initialwert bei t=0
                        manual_value = details["manual_initialization"][input_index]
                        print(
                            f"[t={t}] Using manual initialization for {network_name}:{output_name} → {manual_value}"
                        )
                        inputs.append(np.array([[manual_value]]))
                    elif output_name in network_outputs[network_name]:
                        # Fallback: letztes verfügbares t-1
                        print(
                            f"[t={t}] Using fallback (t-1) from {network_name}:{output_name}"
                        )
                        inputs.append(
                            network_outputs[network_name][output_name][t - 1 : t]
                        )
                    else:
                        raise ValueError(
                            f"Missing input '{output_name}' from network '{network_name}' at timestep {t}"
                        )
                else:
            ...
            if isinstance(skip_cfg, (list, tuple)):
                ...
                if np.allclose(current_input, 0.0):
                    print(
                        f"[t={t}] Skipping {name} because ALL inputs are zero (per config)"
                    )
                    predictions = np.zeros((1, len(details["outputs"])))
                    time_one_step[name].append(0.0)
                    did_skip = True
            ...
            if not did_skip:
                ...
                if is_stateful:
                    ...
                    if t == 0:
                        aux_initial_values, outputs_for_logging = (
                            aux_initial_true_values(
                                details, true_outputs, output_name_init
                            )
                        )
                        all_initial_outputs_logged.update(outputs_for_logging)

                    predictions = predict_with_lstm_network(
                        model_main,
                        model_init,
                        input_scaler,
                        output_scaler,
                ...
                if predictions is None:
                    print(
                        f"[t={t}] Prediction failed in {name}; filling zeros for this step."
                    )
                    predictions = np.zeros((1, len(details["outputs"])))
            ...
            for idx, output_name in enumerate(details["outputs"]):
                if output_name not in network_outputs[name]:
                    ...
                    network_outputs[name][output_name] = np.append(
                        network_outputs[name][output_name],
                        predictions[:, idx : idx + 1],
                        axis=0,
                    )

    avg_prediction_time = {
        name: np.mean(times[1:]) for name, times in time_one_step.items()
    }
...
def calculate_max_re(predictions, true_values):
    ...
    mask = np.isfinite(predictions) & np.isfinite(true_values) & (true_values != 0)
    return (
        np.max(np.abs((true_values[mask] - predictions[mask]) / true_values[mask]))
        * 100
    )
...
def add_predictions_to_dataset(
    df, config, final_outputs, maes, mapes, max_aes, max_res, cum_true, cum_pred
):
    new_cols = {}
    for network_name, details in config.items():
        for output_name in details["outputs"]:
            base = f"{output_name}"
            if (
                network_name in final_outputs
                and output_name in final_outputs[network_name]
                and final_outputs[network_name][output_name].size > 0
            ):
                new_cols[f"{base}_pred"] = final_outputs[network_name][
                    output_name
                ].ravel()
            ...
            if network_name in maes and output_name in maes[network_name]:
                ...
                new_cols[f"{base}_cum_true"] = cum_true[network_name][
                    output_name
                ].ravel()
            if network_name in cum_pred and output_name in cum_pred[network_name]:
                new_cols[f"{base}_cum_pred"] = cum_pred[network_name][
                    output_name
                ].ravel()
    if new_cols:
...
def main(config, input_base, output_dir="Output"):
    ...
    df, initial_inputs, true_outputs, frequency, max_time, time_y = (
        load_inputs_outputs_from_csv(csv_path, config)
    )
    final_outputs, avg_prediction_times, total_time, outputs_for_logging = (
        run_chained_networks(config, initial_inputs, true_outputs)
    )

    # Neuen Unterordner mit Zeitstempel für die Ergebnisse erstellen
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = os.path.join(
        output_dir, f"{timestamp}_results_{os.path.splitext(input_base)[0]}"
    )
    ...
    for name, details in config.items():
        ...
        for output_column in details["outputs"]:
            if output_column in true_outputs:
                ...
                cum_tru, cum_pre, cum_err, cum_rel_err = cumulative_absolute_error(
                    predictions, true_values, frequency
                )
                ...
                valid_indices = (
                    ~np.isnan(true_values)
                    & ~np.isinf(true_values)
                ...
                fig.write_html(
                    os.path.join(
                        results_dir, f"{name}_{output_column}_interactive.html"
                    )
                )
                ...
                plot_path = os.path.join(
                    results_dir, f"{name}_{output_column}_scatter.png"
                )
                plt.tight_layout()
                plt.savefig(plot_path)
                plt.close()
    ...
    with open(f"{results_dir}/Auswertung.txt", "w") as file:
        ...
        for name, avg_time in avg_prediction_times.items():
            file.write(
                f"Average prediction time for {name}: {round(avg_time, 5)} seconds\n"
            )
        file.write(f"\n")
        file.write(
            f"Total prediction time for the entire process: {round(total_time, 5)} seconds\n\n"
        )
    ...
    df_with_predictions = add_predictions_to_dataset(
        df, config, final_outputs, maes, mapes, max_aes, max_res, cum_true, cum_pred
    )
    ...
    print(
        f"Dataset with predictions, MAE, MAPE, Max AE, and Max RE saved to {output_csv_path}"
    )
    ...
    for net_name in cum_error.keys():
        for sort, err in error.items():
            for out_para, wert in err[net_name].items():
                all_evu.setdefault(datei, {}).setdefault(net_name, {}).setdefault(
                    sort, {}
                )[out_para] = wert
...
def all_eval_in_csv(all_evu, output_dir="Output"):
    ...
    mean_matrix = (
        df.groupby(["NN", "Metrics", "Parameter"])["Wert"].mean().reset_index()
    )
    ...
    if input_base.endswith(".csv"):
        ...
        for datei in os.listdir(data_path):
            if datei.endswith(".csv"):
                ...
                all_evu = all_eval(
                    all_evu, datei, cum_error, cum_rel_error, maes, mapes
                )
