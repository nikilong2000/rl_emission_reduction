...
class PredictiveController:
    ...
    def load_cycle(self,str_dir_cycles,file_cycle):
        ...
        self.vect_ac=self.vect_errors/3.6 #Acceleration in SI units, timestep is 1s
    ...
    def set_gains(self,K_split,PCI,rend): #default start values for first engine run
    ...
    def power_dynamics(self):
        ...
        T_whl = Rroll * (mass * acc + mass * g * f * math.cos(theta) + mass * g * math.sin(theta) + Cd * A * (v_ms)**2 / 21.15) # Torque, N·m
    ...
    def box_engine_ctrl(self):
        self.m_inj = max(0, self.P_ice * self.rend) # Injection mass in grams per second (gps)
    ...
    def if_then_ctrl(self):
        ...
        if ((ac_ped < 15.0) and (v_current >= 10.0)) or ((ac_ped > 15.0) and (v_current >= 10.0)):
            n_ice_sp=2100
        else:
            n_ice_sp=1800
    ...
    def EMx_power(self):
        ...
        T_EM2 = 1 / (k1 + k2 + 1) * (T_whl * k1 / kf - (k1 + 1) * T_ice) #Formulas from the paper, steady-state eCVT2 with EM1 as speeder
