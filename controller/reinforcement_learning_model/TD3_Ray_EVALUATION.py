...
class RolloutWorker:
    ...
    def collect_windows(self):
        ...
        return (
            ...
            total_duration,
            self.worker_id,
            action_logic_duration_total,
            env_step_duration_total,
            ice_duration_total,
            pg_duration_total,
        )
...
class TD3:
    ...
    def learn(
                ...
                self.profiling_data.append(
                    {
                        ...
                        "worker_id": worker_id,
                        "timestamp": time.time(),
                        "action_logic_duration": action_t,
                        "env_step_duration": step_t,
                        "ice_duration": ice_t,
                        "pg_duration": pg_t,
                    }
                )
        ...
        while timesteps_collected < total_timesteps:
            if (
                ...
                print(
                    f"Training iterations: {self.total_iterations} | Timesteps: ~{timesteps_collected}/{total_timesteps} | Buffer: {len(self.replay_buffer)}"
                )
            ...
            if updates_done_since_eval >= gradient_steps:
                print(f"\n--- Periodic Evaluation Burst ---")

                eval_start_time = time.time()
                current_eval, avg_action_t, avg_step_t, avg_ice_t, avg_pg_t = (
                    self.eval_episodes(n=self.num_workers)
                )
                    ...
                    self.profiling_data.append(
                        {
                            ...
                            "worker_id": "learner",
                            "timestamp": time.time(),
                            "action_logic_duration": avg_action_t,
                            "env_step_duration": avg_step_t,
                            "ice_duration": avg_ice_t,
                            "pg_duration": avg_pg_t,
                        }
                    )
    ...
    # Inside the TD3 class, replace this entire method:
    # Inside the TD3 class, replace this entire method:

    def _analyze_profiling_data(self):
        """
        Processes the collected timing data, prints statistics, and generates four charts
        with the requested visualization improvements.
        """
        if not self.profiling_data:
            print("\n[PROFILING] No profiling data collected.")
            return
        ...
        print(f"\n[PROFILING] Bottleneck analysis chart saved in: {plot_path_box}")
        ...
        if not df_learner.empty:
            ...
            if total_learner_time > 0:
                ...
                print("\n[PROFILING] Learner time percentage by activity:")
                ...
                print(f"\n[PROFILING] Learner activity chart saved in: {plot_path_pie}")
        ...
        if not df_coll.empty:
            ...
            print(
                "\n[PROFILING] Average duration of Data Collection components by Worker (s):"
            )
            ...
            fig_bar = px.bar(
                ...
                y="average_duration",
                color="component",
                title="Average Data Collection Time Breakdown by Worker",
                labels={
                    "worker_id": "Worker ID",
            ...
            print(
                f"\n[PROFILING] Data Collector breakdown chart saved in: {plot_path_bar}"
            )

        # --- Detailed Analysis of Evaluation (Average Episode) ---
        ...
        if not df_eval.empty and "action_logic_duration" in df_eval.columns:
            ...
            print("\n[PROFILING] Time breakdown of an AVERAGE evaluation episode (s):")
            print(
            ...
            bar_data_eval = pd.DataFrame(
                {
                    ...
                    "episode": "Average Evaluation Episode",
                }
            )
            fig_bar_eval = px.bar(
                bar_data_eval,
                ...
                y="duration",
                color="component",
                title="Time Breakdown of an Average Evaluation Episode",
                labels={
                    "episode": "",
            ...
            print(
                f"\n[PROFILING] Evaluation breakdown chart saved in: {plot_path_bar_eval}"
            )

        # --- Detailed Analysis of Simulation (env.step) ---
        ...
        if not df_sim.empty and "ice_duration" in df_sim.columns:
            ...
            print(
                "\n[PROFILING] Average duration of env.step components by event type (s):"
            )
            print(sim_analysis)
            ...
            fig_bar_sim = px.bar(
                ...
                title="Average Simulation Time Breakdown (env.step)",
                labels={
                    "event": "Context",
                    "average_duration": "Average Duration (s)",
            ...
            print(
                f"\n[PROFILING] Simulation breakdown chart saved in: {plot_path_bar_sim}"
            )
    ...
    def eval_episodes(self, n: int = 3):
        ...
        return (
            ...
            avg_action_logic_duration,
            avg_env_step_duration,
            avg_ice_duration,
            avg_pg_duration,
        )
    ...
    def _init_or_check_buffer_meta(self):
        ...
        if not os.path.exists(self._buffer_meta_path):
            ...
            try:
                ...
                if not ok:
                    print(
                        f"[WARN] El buffer en disco fue creado con otra configuración (meta={old}). Se seguirá, pero podrías tener incompatibilidades."
                    )
