...
import pandas as pd
import os
import time # <-- CORRECCIÓ: Importació afegida
...
class Environment:
    ...
    def step(self, action, vel_target: float = 70.0, profile_step: bool = False):
        ...
        if profile_step: ice_start = time.time()
        ...
        if profile_step: ice_duration = time.time() - ice_start
        ...
        if profile_step: pg_start = time.time()
        ...
        if profile_step: pg_duration = time.time() - pg_start
